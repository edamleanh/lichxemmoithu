  ...
  async fetch({ from, to }) {
    try {
      ...
      try {
        ...
        const testResponse = await fetch('https://api.football-data.org/v4/competitions', {
          headers: { 'X-Auth-Token': '354c9341dac74c788f59795973d8099d' },
      ...
      const leaguePromises = leagues.map(async (league) => {
        try {
          ...
          const response = await fetch(
            `https://api.football-data.org/v4/competitions/${league.id}/matches?dateFrom=${dateFrom}&dateTo=${dateTo}`,
            { headers: { 'X-Auth-Token': '354c9341dac74c788f59795973d8099d' } }
          )
